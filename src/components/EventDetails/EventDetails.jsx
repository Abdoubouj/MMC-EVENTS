...
const EventDetails = () => {
    ...
    const events = useSelector((state)=>state.event.events.filter((e)=>e.evenementId !== event?.evenementId));
  ...
  return (
    <div className="event-details">
        <main className="event-details-left">
            <h1 className='event-title'>{event?.titre}</h1>
            <img src={event.photo} className='event-image' alt="#" />
            <div className="meta-info">
            {/* <p className='event-adresse'>{event?.adresse},{event?.city}</p> */}
            <p className='event-date'>
                <span>{dateFormat(event?.dateDebut)}</span>
                <span>/</span>
                <span>{dateFormat(event.dateFin)}</span>
            </p>
            </div>
            ...
            <div className="sessions">
                ...
                <div className="d-flex gap-3">
                 {eventSessions.map((session)=>(
                     <div className='card' style={{width:"200px"}} key={session.sessionId}>
                    ...
                    {events?.map((event)=>(
                        <div className="recommended-event-item" key={event.evenementId}>
                            ...
                            <div className="recommended-event-info">
                              ...
                              <Link to={`/events/${event.evenementId}`}>Read more <KeyboardDoubleArrowRightIcon/></Link>
                            </div>
                        </div>
                    ))}
                </div>
